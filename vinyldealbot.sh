...
case "$1" in
  start)
    ...
    # Start the daemon
    python /usr/share/vinyldealbot/vinyldealbot.py start
    ;;
  stop)
    echo "Stopping server"
    # Stop the daemon
    python /usr/share/vinyldealbot/vinyldealbot.py stop
    ;;
  restart)
    echo "Restarting server"
    python /usr/share/vinyldealbot/vinyldealbot.py restart
    ;;
  *)
    # Refuse to do other stuff
    echo "Usage: /etc/init.d/vinyldealbot.sh {start|stop|restart}"
    exit 1
    ;;
esac
